// Holdings strip: one tile per symbol, width proportional to market value, with
// the unrealised return spelled out. Tiles pulse when the price ticks.
...
export class Heatmap {
  ...
  render(holdings: Holding[], priceDirections: Record<string, "up" | "down" | "same">): void {
    ...
    for (const h of holdings) {
      ...
      tile.querySelector(".heatmap-pct")!.textContent = formatReturn(h);
  ...
  private createTile(symbol: string): HTMLElement {
    ...
    tile.innerHTML = `
      <div class="heatmap-row">
        <span class="heatmap-ticker"></span>
        <span class="heatmap-pct"></span>
      </div>
      <span class="heatmap-value"></span>
    `;
    tile.querySelector(".heatmap-ticker")!.textContent = symbol;
    tile.title = symbol;
    return tile;
  }
}
...
function formatReturn(h: Holding): string {
  const cost = h.avg_cost * h.quantity;
  if (cost <= 0) return "";
  const pct = (h.unrealized_pnl / cost) * 100;
