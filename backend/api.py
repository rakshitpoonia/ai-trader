...
def average_cost(account: Account, symbol: str) -> float:
    ...
    spend = sum(t.price * t.quantity for t in account.transactions if t.symbol ==
                symbol and t.quantity > 0)
    bought = sum(t.quantity for t in account.transactions if t.symbol ==
                 symbol and t.quantity > 0)
...
def get_trader_logs(name: str, last_n: int = 13) -> list[dict]:
    ...
    return [
        {"datetime": ts, "type": kind, "message": message,
